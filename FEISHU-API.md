...
| # | 分类 | 方法 | 路径 / 帧类型 | 实现位置 |
|---|------|------|---------------|----------|
| 1 | 鉴权 | POST | `/open-apis/auth/v3/tenant_access_token/internal` | `internal/lark/auth.go:114` |
| 2 | 消息发送（新建） | POST | `/open-apis/im/v1/messages?receive_id_type=chat_id` | `internal/lark/rest.go:47` |
| 3 | 消息发送（回复） | POST | `/open-apis/im/v1/messages/{message_id}/reply` | `internal/lark/rest.go:58` |
| 4 | 消息更新 | PATCH | `/open-apis/im/v1/messages/{message_id}` | `internal/lark/rest.go:106` |
...
| 5 | WebSocket 引导 | POST | `/callback/ws/endpoint` | `internal/lark/ws/wsclient.go:264` |
| 6 | WebSocket 拨号 | GET（Upgrade） | 服务器签名返回的 wss URL | `internal/lark/websocket/dial.go:49` |
| 7 | 收事件 `im.message.receive_v1` | WS Binary 帧 | — | `internal/lark/ws/dispatcher.go:174` |
| 8 | 卡片回调 `card.action.trigger` | WS Binary 帧 | — | `internal/lark/ws/dispatcher.go:180` |
| 9 | Ping 心跳 | WS Binary 帧 | — | `internal/lark/ws/session.go:244` |
| 10 | ACK 事件回执 | WS Binary 帧 | — | `internal/lark/ws/session.go:227` |

**域名**（`resolveBaseURL`，`internal/lark/client.go:30-41`）：空/`"feishu"` → `https://open.feishu.cn`；`"larksuite"`/`"lark"` → `https://open.larksuite.com`；带 scheme 原样；裸 host → `https://<host>`。三个 baseURL（REST/Token/WS bootstrap）在 `NewClient` 用同一个值构造（`internal/lark/client.go:71-91`）。bootstrap 返回的 wss URL 由飞书签名，域名随调用域。
...
- **HTTP 方法**：`POST`（两路径都是）
- **完整 URL**：
  - **新建**：`{baseURL}/open-apis/im/v1/messages?receive_id_type=chat_id`（`internal/lark/rest.go:47-67`）
  - **回复**：`{baseURL}/open-apis/im/v1/messages/{ReplyMessageID}/reply`（无 query；`ReplyMessageID` 经 `url.PathEscape`；`:57`）
- **请求结构**（`internal/lark/rest.go:47-77`）：
...
- **调用点**：REST 核心 `restClient.SendMessage`（`:46-75`）；底层 `doJSON`（`:110-154`）；公共出口 `Client.Send`（`internal/lark/client.go:97-138`）；业务封装 `Bot.SendCard`（`internal/feishu/bot_send.go:54-85`）、`Bot.SendText`（`:92-109`）。
- **上游业务用途**：
...
- **分类**：消息更新
- **HTTP 方法**：`PATCH`
- **完整 URL**：`{baseURL}/open-apis/im/v1/messages/{message_id}`（无 query；`messageID` 经 `url.PathEscape`；`internal/lark/rest.go:106`）
- **请求结构**（`:84-85`）：
...
- **调用点**：REST 核心 `restClient.PatchMessage`（`:79-86`）；公共出口 `Client.PatchMessage`（`internal/lark/client.go:97-143`）；业务封装 `Bot.UpdateCard`（`internal/feishu/bot_send.go:113-167`，带退避重试 `:134-162`）、`Bot.updateFallbackCard`（`:171-177`）。
- **上游业务用途**：
  - 长任务进行中：进度卡片反复刷新。
...
- **请求结构**（`internal/lark/ws/wsclient.go:262-211`）：
...
- **错误码处理**（`:238-244`）：
  ...
  | 1 | `codeSystemBusy` | 定义于 `proto.go:38`，未特判，按瞬态重连 |
...
- **错误码处理**：
  ...
  - TLS/NET 拨号失败 → `websocket: dial: %w`（`:88-90`）；上游包装 `ws: dial: %w`（`wsclient.go:254`）。
...
- **WS 帧类型**：Binary（`OpcodeBinary=0x2`），`Method=MethodData=1`，`Headers.type=TypeEvent="event"`（`internal/lark/ws/proto.go:8-11,24-28`）
- **帧结构**（protobuf `Frame`，`internal/lark/ws/frame.go:25-34`）：
...
- **Payload 业务 JSON**——v2 信封（`envelope`，`internal/lark/ws/dispatcher.go:152-155`）：
...
- **调用点 / 解析点**：帧解码 `Frame.Unmarshal`（`frame.go:148-278`）；路由分支 `dispatcher.go:174-178`；payload 解析 `parseMessageReceive`（`:212-249`）；透传 `Sink.OnMessage`（`:173`）；adapter `handlerSinkAdapter.OnMessage`（`client.go:221-235`）；业务消费 `Bot.handleMessageReceive`（`internal/feishu/bot_dispatch.go:34-99`）。
...
- **错误码处理**：
  ...
  - 信封/事件 JSON 解析失败：`dispatch` 返回 err → ACK 500（`dispatcher.go:170-171,186-187`）。
...
- **WS 帧类型**：同 §6
- **帧结构**：同 §6
- **Payload 结构**：v2 信封同 §6，但 `header.event_type == "card.action.trigger"`，`event` 体为 `cardEvent`（`dispatcher.go:281-293`）：
...
- **调用点 / 解析点**：路由分支 `dispatcher.go:180-184`；解析 `parseCardAction`（`:267-282`）；透传 `Sink.OnCard`（`:179`）；adapter `handlerSinkAdapter.OnCard`（`client.go:237-251`）；业务消费 `Bot.handleCardAction`（`bot_dispatch.go:105-142`）。
...
- **Payload**（`session.go:227`）：`{"code": <ackCode>}`
...
- **分类**：WebSocket 事件接收（出站控制）
- **WS 帧类型**：Binary，`Method=MethodControl=0`，`Headers.type=TypePing="ping"`，`Service=c.serviceID`（`NewPingFrame`，`frame.go:77-95`）
- **Payload**：空
...
- `reconnectSleep`（`reconnect.go:14-35`）：先 `[0, ReconnectNonce)` 抖动，再睡 `ReconnectInterval`；ctx 取消返回 false。
...
### 11.3 读超时（半开检测）
- `refreshReadDeadline`（`session.go:117-133`）：每次成功读到帧后把读 deadline 推 `2 * PingInterval`。
...
### 11.5 chunk 重组
- `reassembler`（`dispatcher.go:76-135`）：按 `message_id`/`sum`/`seq` 重组分片 data 帧。
...
| 项 | 位置 |
|----|------|
| `Frame` 结构与字段字号注释 | `internal/lark/ws/frame.go:10-34` |
| `Header` 结构 | `internal/lark/ws/frame.go:39-41` |
| `Headers` 线性查找 helper | `internal/lark/ws/frame.go:50-73` |
| `Marshal`（手写 protobuf 编码器） | `internal/lark/ws/frame.go:113-149` |
| `Unmarshal`（手写解码器，跳过未知字段） | `internal/lark/ws/frame.go:148-285` |
| varint/length-delimited 原语 | `internal/lark/ws/frame.go:399-422` |
| `skipField`（未知字段跳过） | `internal/lark/ws/frame.go:280-352` |
| 头部上限 `maxFrameHeaders = 64` | `internal/lark/ws/frame.go:47` |
...
- `resolveBaseURL`（`internal/lark/client.go:30-41`）：
...
| 符号 | 位置 | 状态 |
|------|------|------|
| `codeSystemBusy = 1` | `internal/lark/ws/proto.go:38` | 定义但 bootstrap 未特判，按通用瞬态错误重连 |
...
| `TypeCard = "card"` | `internal/lark/ws/proto.go:24` | 注释明示 "legacy card channel (unused by this app)" |
...
feishu.NewBotWithLogger                                   internal/feishu/bot.go:120
  └─ lark.NewClient                                       internal/lark/client.go:75
       ├─ tokenManager{baseURL,http}                      internal/lark/client.go:71
       ├─ restClient{baseURL,http,tokens}                 internal/lark/client.go:88
       └─ ws.New(appID,appSecret,baseURL,httpc)           internal/lark/client.go:75 → ws/wsclient.go:103
...
Bot.Start → client.Start → ws.Start                       internal/feishu/bot.go:212 → lark/client.go:104 → ws/wsclient.go:129
  ...
  └─ runSession                                           ws/session.go:22
       ...
       │    ├─ Frame.Unmarshal                            ws/frame.go:148
       │    ├─ handleControl (pong→刷新 cfg)              ws/session.go:132
       │    └─ handleData                                 ws/session.go:162
       │         ├─ reassembler.feed                      ws/dispatcher.go:103
       │         ├─ router.dispatch                       ws/dispatcher.go:165
       │         │    ├─ im.message.receive_v1 → Sink.OnMessage   ws/dispatcher.go:175

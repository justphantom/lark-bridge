...
- claude-back：`/running` `/session` `/new` `/abort` `/session-del` `/current` `/model` `/cd` `/settings` `/perm` `/effort` `/send` `/pull` `/push` `/help`。
- miniagent-back：`/current` `/model` `/models` `/cd` `/new` `/send` `/pull` `/push` `/running` `/abort` `/help`。

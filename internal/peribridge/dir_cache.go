...
// validateWorkspacePath checks that dir is an immediate or nested subdirectory
// of workspaceRoot, refusing escapes.
...
// Forms:
//   - /cd            → pop a selection card listing WORKSPACE_ROOT subdirs
//   - /cd clear      → clear the pin (fall back to the per-chat default dir)
//   - /cd <path>     → pin <path> directly (must be under WORKSPACE_ROOT)
//
// peri is stateless, so changing the directory needs no session reset — the
// next prompt simply runs in the new cwd.
func (h *Handler) cmdDirectory(_ context.Context, chatID string, args []string) (commandResult, error) {
	...
	if dir == filepath.Clean(b.Directory) {
		return commandResult{Body: "目录未变化。"}, nil
	}

	h.abortChat(chatID)
	h.router.SetDirectory(chatID, dir)
	cmdutil.LogSettingChange(h.logger, chatID, "directory", dir)
	return cmdutil.ChangeResult("工作目录", b.Directory, dir, "下次提问生效。"), nil
}

// runDirPicker drives the interactive directory selection.
func (h *Handler) runDirPicker(chatID, oldDir string) commandResult {
	...
	if dir == filepath.Clean(oldDir) {
		h.emitNoticeLogged(chatID, "info", "目录未变化", "选中的目录与当前一致。")
	...
	h.router.SetDirectory(chatID, dir)
	cmdutil.LogSettingChange(h.logger, chatID, "directory", dir)
	res := cmdutil.ChangeResult("工作目录", old, dir, "下次提问生效。")
...
// to its per-chat default directory.
func clearDirectory(h *Handler, chatID, oldDir string) commandResult {
	...
	return cmdutil.ChangeResult("工作目录", old, "(默认)", "已清除目录设置，下次提问生效。")
}

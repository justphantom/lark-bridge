...
// cancelAnswer removes a pending slot without delivering an answer.
func (h *Handler) cancelAnswer(requestID string) {
	h.answers.Cancel(requestID)
}

// deliverAnswer routes an inbound answer to the goroutine waiting on requestID,
// if any. Returns false when no waiter exists.
func (h *Handler) deliverAnswer(requestID string, ans *protocol.AnswerPayload) bool {
	return h.answers.Deliver(requestID, ans)
}

// drainAnswers closes every pending answer slot so any blocked picker returns
// immediately. Called by Close.

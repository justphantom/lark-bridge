...
func (h *Handler) HandleEvent(ctx context.Context, ev *protocol.Event) error {
	...
	case protocol.TypeAnswer:
		// An interactive card reply (model/approval/thinking picker). Route
...
// handlePromptEvent resolves/creates the chat binding, then launches
...
func (h *Handler) handlePromptEvent(ctx context.Context, ev *protocol.Event) error {
	...
	if field := p.HasFrontendOverride(); field != "" {
		return h.Emit(ctx, replyToID, &protocol.Control{
			...
			Notice: &protocol.NoticePayload{
				Level:   "error",
				Title:   "协议违规",
				Message: "前端不允许设置 " + field + " 字段；请通过 /cd /model /perm /thinking 等命令调整。",
			},
		})
	}
	...
	// project yet. Intercept before starting the CLI: running without a dir

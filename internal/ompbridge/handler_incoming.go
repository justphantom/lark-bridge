...
// applies any per-prompt overrides carried by the Event (sessionID,
// directory, modelSpec). omp sessions are lazy: the binding starts with an
// empty session id (filled from the first run's `session` header by
// streamRun) and a per-chat working directory under the configured default
// directory.
//
...
func (h *Handler) ensureBinding(chatID, sessionID, directory, modelSpec, title string) (binding router.Binding, err error) {
	...
	// MkdirAll so an untrusted source cannot make the subprocess CWD escape
	// the intended tree (mirrors /cd's validateAbsDir, but without the
	// existence check — the dir is created on demand below).
	if directory != "" {

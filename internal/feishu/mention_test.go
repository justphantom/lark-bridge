...
func TestStripMentionPlaceholders(t *testing.T) {
	bot := Mention{Key: "@_user_1", Name: "BridgeBot", OpenID: "bot-open-id", IsBot: true}
	alice := Mention{Key: "@_user_2", Name: "Alice", OpenID: "alice-open-id"}
	bob := Mention{Key: "@_user_3", Name: "Bob", OpenID: "bob-open-id"}
	...
	t.Run("bot mention removed by IsBot flag", func(t *testing.T) {
		...
		got := StripMentionPlaceholders(text, []Mention{bot})
		if got != "please help" {
	...
	t.Run("user mention replaced with name", func(t *testing.T) {
		...
		got := StripMentionPlaceholders(text, []Mention{alice})
	...
	t.Run("user with empty name falls back to 用户", func(t *testing.T) {
		anon := Mention{Key: "@_user_2", OpenID: "x"}
		text := "@_user_2 hi"
		got := StripMentionPlaceholders(text, []Mention{anon})
		if got != "@用户 hi" {
	...
	t.Run("longest-placeholder-first avoids prefix corruption", func(t *testing.T) {
		...
		u1 := Mention{Key: "@_user_1", Name: "First", OpenID: "id1"}
		u10 := Mention{Key: "@_user_10", Name: "Tenth", OpenID: "id10"}
		text := "@_user_10 and @_user_1"
		got := StripMentionPlaceholders(text, []Mention{u1, u10})
	...
	t.Run("all-hands mention removed", func(t *testing.T) {
		all := Mention{Key: "@_all", OpenID: "all"}
		text := "@_all team update"
		got := StripMentionPlaceholders(text, []Mention{all})
		if got != "team update" {
	...
	t.Run("mixed bot + users", func(t *testing.T) {
		...
		got := StripMentionPlaceholders(text, []Mention{bot, alice, bob})

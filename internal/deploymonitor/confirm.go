...
func (h *Handler) confirmAndDeployForce(ctx context.Context, chatID, promptID string) error {
	requestID := promptID
	...
	if !ok {
		...
		return h.notify(ctx, chatID, promptID, "warning", "等待确认",
...
// bound to promptID). The confirm card's own messageID is distinct from the
// progress card's; after the click the confirm card stays in its "submitted"
// state (the deploy notice lands on the progress card, not the confirm card).
// Cancel/timeout releases the wait with a bound terminal notice.
func (h *Handler) awaitForceConfirm(chatID, promptID string, ch <-chan *protocol.AnswerPayload) {
	confirmCtx, cancel := context.WithTimeout(context.Background(), confirmTimeout)
	defer cancel()
	select {
	case ans, ok := <-ch:
		if !ok || ans == nil || ans.Choice != "confirm" {
			h.notifyWithRetry(chatID, promptID, "info", "已取消", "强制部署已取消，未执行任何操作。")
			return
		}
		if err := h.acquireAndRun(confirmCtx, chatID, promptID, "make", h.deployArgs(true), "部署"); err != nil {
			h.notifyWithRetry(chatID, promptID, "error", "部署失败", "启动部署失败："+err.Error())
		}
	case <-confirmCtx.Done():
		h.answers.Cancel(promptID)
		h.notifyWithRetry(chatID, promptID, "warning", "确认超时",

...
func (c *Client) pump(ctx context.Context, cmd *exec.Cmd, stdout, stderr io.Reader, out chan<- Event, sink io.Writer) {
	...
	go func() {
		...
		// / idle_timeout=0 there is no fallback cancel). The head is already
		// captured for diagnostics; the tail is discarded. Matches
...
ScanLoop:
	for {
		...
		if err != nil {
			c.logger.Warn("read claude stdout", "error", err)
			break
		}

		if truncated {
			eventmetrics.LineTruncated("claude").Inc()
			c.logger.Warn("claude stream line truncated", "kept_len", len(line), "max", maxLineLen)
		}
...
// truncate has been unified to strutil.Truncate (byte-identical impl).

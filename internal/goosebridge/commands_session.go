...
// fresh goose conversation. The working directory is preserved (files
...
// cmdSessionAbort cancels the in-flight goose turn for this chat, if any.
func (h *Handler) cmdSessionAbort(_ context.Context, chatID string, _ []string) (commandResult, error) {
	if h.abortChat(chatID) {
		return commandResult{Body: "已中止当前 goose 调用。"}, nil
	}
	return commandResult{Body: "当前没有正在执行的 goose 调用。"}, nil
}

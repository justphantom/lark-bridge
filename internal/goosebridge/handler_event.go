...
// handlePromptEvent resolves/creates the chat binding (carrying goose
// per-prompt overrides Directory/ModelSpec onto the binding), then launches
...
//
// goose has no CLI flags for permission mode, effort, or settings files, so
// those PromptPayload fields are intentionally ignored (unlike claude-back).

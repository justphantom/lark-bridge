...
// Handler is the goose-back orchestrator. One per process. It owns the
// router (chatID → goose session binding), the goose client, and the
...
type Handler struct {
	...
	// permissionDefault is the process-level permission mode shown in
	// /current when no per-chat pin is set. Retained for display parity
	// with the other backends; goose has no --permission-mode flag of its
	// own, so an operator-configured default is informational only.
...
func NewWithLogger(r sessionRouter, api gooseAPI, rpc *backendrpc.Client, cfg HandlerConfig, logger *log.Logger) *Handler {
	if logger == nil {
		logger = log.Nop()
	}
	h := &Handler{

...
func TestSummarizeToolInput_Subject(t *testing.T) {
	...
	if got := summarizeToolInput(input); got != "梳理架构" {
...
func TestSummarizeToolInput_FilePath(t *testing.T) {
	...
	if got := summarizeToolInput(input); got != "/opt/codes/README.md" {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := summarizeToolInput(tc.input); got != tc.want {
				t.Errorf("summarizeToolInput(%s) = %q, want %q", tc.name, got, tc.want)
			}
		})
	}
}
...
func TestSummarizeToolInput_FirstStringValueFallback(t *testing.T) {
	...
	if got := summarizeToolInput(input); got != "bar" {

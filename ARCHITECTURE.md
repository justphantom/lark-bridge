...
- **316 个 Go 文件，约 66,950 行**，其中生产代码 **约 34,788 行**，测试代码 **约 32,162 行（约 48.0%）**
- `internal/` 下 **26 个子包**（`go list` 计 31 个含嵌套）
...
- v1.3.0：自实现飞书客户端 → `internal/lark/`，最终实现"零外部依赖"
...
- **二进制数**：5（cmd/ 下 5 子目录）
- **internal 包数**：26（顶层）/ 31（含嵌套）
- **Go 文件**：316 个，约 66,950 行（生产代码约 34,788 行，测试代码约 32,162 行，约 48.0%）
- **外部依赖**：0
- **版本**：v1.14.0
- **License**：MIT（LICENSE）

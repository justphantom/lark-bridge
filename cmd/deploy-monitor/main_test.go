...
// is the only part of main worth pinning without spawning `make` against
// a real repo; happy paths are covered by internal/deploymonitor tests.
func TestRun_BadConfigReturnsError(t *testing.T) {
	if err := run("/nonexistent/lark-deploy-monitor-config.json"); err == nil {

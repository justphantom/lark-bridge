...
layer: L2
type: incident
tags: [feishu, card, patch, race-condition, terminal-state, fingerprint, bounce-back]
created: 2026-08-09
confidence: high
---
...
5. **终态标记是处理“延迟写覆盖新写”的通用模式**：写者延迟不可控时，让后到写者落地前检查“目标是否已被更新的写终结”，比重排时序和读回判断都干净。已推广为全项目模式：[patterns/card-terminal-state-guard.md](../patterns/card-terminal-state-guard.md)。

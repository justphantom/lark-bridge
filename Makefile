...
#
# Targets:
#   build       compile the three binaries into bin/ (version-stamped)
...
#   pack        build all three binaries and bundle into a distributable tarball
...
# pack 交叉编译三个二进制 + VERSION 标记，打成一个可分发的 tarball。
# 在临时 staging 目录构建，避免 bin/ 里已有的旧 tarball/二进制被卷进新包。
# 输出 bin/lark-bridge-<version>-<goos>-<goarch>.tar.gz，解包后顶层即各二进制。
pack:
	@tmp=$$(mktemp -d) && trap "rm -rf $$tmp" EXIT; \
	mkdir -p bin; \
	for name in lark-feishu-front:cmd/feishu-front lark-miniagent-back:cmd/miniagent-back lark-status-monitor:cmd/status-monitor; do \
		out=$${name%%:*}; src=./$${name##*:}; \
